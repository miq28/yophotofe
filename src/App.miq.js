...
import { toastError, toastSuccess } from '../src/redux/actions/toastActions';
...
function App() {
  ...
  useEffect(() => {
    ...
    if (jwtToken) {
      ...
      if (Date.now() <= decoded.exp) {
        message = `Nice, token will still be valid for the next ${Math.round(decoded.exp - Date.now() / 1000)} seconds 🤗`
        console.log(message)
        toastSuccess(message)
        loginState = true;
      }
      else {
        message = 'Oh no.. token has expired 😮😬. You have to relogin.'
        console.log(message)
        toastError(message)
        loginState = false;
      }
    ...
    // axios.get(`${URL_API}/users/profile/userid/${decoded.id}`)
    //     .then((res) => {
  ...
  // if (CASE === 2) window.location = '/expired'
    ...
    return (
      <>
        ...
        <Switch>
          ...
          <Route exact path="/expired" component={TokenExpired} />
          ...
          <Route exact path="/contest" component={NotLogin} />
          <Route exact path="/user/all" component={NotLogin} />
